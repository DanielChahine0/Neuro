    ...
    <div className="rounded-xl bg-surface border border-border overflow-hidden">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs text-muted font-mono uppercase tracking-wider">Active Session</p>
            <p className="text-3xl font-bold font-mono tabular-nums mt-1">02:15:47</p>
          </div>
          <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
            <span className="text-xs font-medium text-emerald-400">In Focus</span>
          </div>
        </div>
        <div className="space-y-2">
          ...
          <div className="w-full h-2 rounded-full bg-border mt-3">
...
function DistractionMockup() {
  const items = [
    { app: "Twitter / X", icon: "🐦", time: "4m 32s", color: "text-red-400" },
    { app: "YouTube", icon: "▶️", time: "2m 18s", color: "text-red-400" },
    { app: "Slack", icon: "💬", time: "1m 45s", color: "text-yellow-400" },
    { app: "Mail", icon: "✉️", time: "0m 52s", color: "text-yellow-400" },
  ];
  return (
    <div className="rounded-xl bg-surface border border-border overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border">
      ...
      <div className="p-6">
        ...
        <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
          ...
          <span className="text-sm font-mono text-red-400 font-medium">9m 27s</span>
        </div>
      </div>
    </div>
  );
            ...
            >
              ...
              <div
                className={`w-10 h-6 rounded-full flex items-center transition-colors ${
                  app.blocked ? "bg-accent justify-end" : "bg-border justify-start"
                }`}
              >
    ...
    <div className="rounded-xl bg-surface border border-border overflow-hidden">
      ...
      <div className="p-6 flex flex-col items-center">
        ...
        <div className="mt-6 w-full grid grid-cols-2 gap-3">
          <div className="px-3 py-2 rounded-lg bg-background/50 text-center">
    ...
    <div className="rounded-xl bg-surface border border-border overflow-hidden">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-xs text-muted font-mono uppercase tracking-wider">This Week</p>
            <p className="text-2xl font-bold mt-1">18h 24m</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted font-mono uppercase tracking-wider">Streak</p>
            <p className="text-2xl font-bold text-accent mt-1">🔥 {streak}d</p>
          </div>
        </div>
        {/* Bar chart */}
        <div className="flex items-end gap-2 h-28">
          {days.map((day, i) => (
            <div key={day + i} className="flex-1 flex flex-col items-center gap-1">
              <div
                className="w-full rounded-md bg-accent/30 hover:bg-accent/50 transition-colors"
          ...
          {features.map((feature, index) => (
            ...
            >
              <div style={{ direction: "ltr" }}>
                <span className="text-6xl font-bold text-border font-mono">
                  {feature.number}
                </span>
